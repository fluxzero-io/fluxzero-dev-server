...
@Component({selector:'dev-startup-page', standalone:true, imports:[StartupComponent], template:`
  <section class="page startup-page" aria-labelledby="startup-title">
    <header class="page-heading"><div><h1 id="startup-title">Startup commands</h1><p>The commands run when your workspace starts.</p></div></header>
    @if(status(); as state) {
      @if(state.maintenance?.workspaceStopped || state.state === 'shutdown') {
        <p class="workspace-stopped" role="status">Workspace stopped. Start it from <a href="#projects">Workspace</a> to prepare startup commands.</p>
      } @else if(state.startup?.actions?.length || state.startup?.state === 'failed') {
        <dev-startup [startup]="state.startup"/>
      } @else {<p class="startup-empty">{{state.state === 'starting' ? 'Waiting for startup actions…' : 'No startup commands configured for this workspace.'}}</p>}
