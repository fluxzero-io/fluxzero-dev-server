      ...
      @for(project of environments();track project.id) {
        <div class="project-row">
          ...
          <div class="row-actions" role="group" [attr.aria-label]="'Actions for ' + project.projectName">
            <button class="icon-button project-action manager-open-action" title="Open project" aria-label="Open project" [disabled]="busy() || project.status !== 'running'" (click)="openProject(project)"><i class="bi bi-box-arrow-up-right" aria-hidden="true"></i></button>
