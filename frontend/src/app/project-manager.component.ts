  ...
  <dialog #dialog aria-labelledby="manage-projects-title" (pointerdown)="backdropPressed = isBackdrop($event)" (click)="dismissBackdrop($event)" (cancel)="cancel($event)" (close)="restoreFocus()" (keydown)="$event.stopPropagation()">
    ...
    @if(mode() === 'list') {
      ...
      @if(removing().length) {<section class="remove-confirm" role="group" aria-label="Confirm removal">
        ...
        <div class="dialog-actions"><button class="secondary-button dialog-cancel" [disabled]="isBusy()" (click)="removing.set([])">Cancel</button><button class="primary-button" [disabled]="isBusy()" (click)="confirmRemoval()">Remove from list</button></div>
      </section>}
      ...
      @if(missing().length) {<button class="secondary-button cleanup" [disabled]="isBusy()" (click)="removing.set(missing())">Remove missing folders from list</button>}
    } @else {
...
`,styles:`
  ...
  .remove-confirm {padding:14px;margin:12px 0;border:1px solid var(--dashboard-border);border-radius:8px;}
